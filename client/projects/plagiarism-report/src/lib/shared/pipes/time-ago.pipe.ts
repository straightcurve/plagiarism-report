...
export class TimeAgoPipe implements PipeTransform, OnDestroy {
  ...
  transform(value: string) {
    ...
    if (Number.isNaN(seconds)) {
      ...
      return timeAgoTranslates && timeAgoTranslates.MINUTE_AGO
        ? timeAgoTranslates.MINUTE_AGO
        : "a minute ago";
    } else if (minutes <= 45) {
      return (
        minutes +
        " " +
        (timeAgoTranslates && timeAgoTranslates.MINUTES_AGO
          ? timeAgoTranslates.MINUTES_AGO
          : "minutes ago")
      );
    } else if (minutes <= 90) {
      return timeAgoTranslates && timeAgoTranslates.HOUR_AGO
        ? timeAgoTranslates.HOUR_AGO
        : "an hour ago";
    } else if (hours <= 22) {
      return (
        hours +
        " " +
        (timeAgoTranslates && timeAgoTranslates.HOURS_AGO
          ? timeAgoTranslates.HOURS_AGO
          : "hours ago")
      );
    } else if (hours <= 36) {
      return timeAgoTranslates && timeAgoTranslates.DAY_AGO
        ? timeAgoTranslates.DAY_AGO
        : "a day ago";
    } else if (days <= 25) {
      return (
        days +
        " " +
        (timeAgoTranslates && timeAgoTranslates.DAYS_AGO
          ? timeAgoTranslates.DAYS_AGO
          : "days ago")
      );
    } else if (days <= 45) {
      return timeAgoTranslates && timeAgoTranslates.MONTH_AGO
        ? timeAgoTranslates.MONTH_AGO
        : "a month ago";
    } else if (days <= 345) {
      return (
        months +
        " " +
        (timeAgoTranslates && timeAgoTranslates.MONTHS_AGO
          ? timeAgoTranslates.MONTHS_AGO
          : "months ago")
      );
    } else if (days <= 545) {
      return timeAgoTranslates && timeAgoTranslates.YEAR_AGO
        ? timeAgoTranslates.YEAR_AGO
        : "a year ago";
    } else {
      return (
        years +
        " " +
        (timeAgoTranslates && timeAgoTranslates.YEARS_AGO
          ? timeAgoTranslates.YEARS_AGO
          : "years ago")
      );
    }
  }
