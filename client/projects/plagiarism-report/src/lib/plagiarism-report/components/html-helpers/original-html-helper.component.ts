...
function deepEqual(original: any, changed: any) {
  return false;
}
...
export class OriginalHtmlHelperComponent
  extends HtmlHelperBase
  implements OnInit, OnDestroy
{
  private lastMatchSelectEvent: MatchSelectEvent | null = null;
  ...
  handleMatchSelect(event: MatchSelectEvent) {
    ...
    if (event.index !== -1)
      this.highlightService.setOriginalHtmlMatch(this.matches[event.index]);
  }
  ...
  ngOnInit() {
    ...
    combineLatest([source$.pipe(truthy()), originalHtmlMatches$])
      ...
      .subscribe(([, matches]) => {
        ...
        if (
          this.lastMatchSelectEvent &&
          this.lastMatchSelectEvent.index !== -1
        ) {
          this.messageFrame({
            type: "match-select",
            index: this.lastMatchSelectEvent?.index,
          } as MatchSelectEvent);
        }
      });

    const onOneToManyHtmlJump$ = combineLatest([
      jump$,
      contentMode$,
      viewMode$,
    ]).pipe(
