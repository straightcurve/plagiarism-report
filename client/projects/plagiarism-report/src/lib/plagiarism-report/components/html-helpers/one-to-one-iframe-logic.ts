...
function ready() {
  ...
  function init() {
    ...
    document.querySelectorAll("span[exclude-partial-scan]").forEach((elem) => {
      elem.addEventListener("click", () =>
        messageParent({ type: "upgrade-plan" })
      );
    });
  }
  ...
  function handleMatchJump(event: MatchJumpEvent) {
    ...
    if (
      (oldGid === firstGid && !event.forward) ||
      (oldGid === lastGid && event.forward)
    ) {
      return;
    }
  ...
  function handleBroadcastMatchSelect(event: MatchSelectEvent) {
    const elem = document.querySelector<HTMLSpanElement>(
      `span[match][data-index='${event.index}']`
    );
  ...
  function handleMatchSelect(
    ...
    if (current && current.dataset.gid === elem.dataset.gid) {
      groups[current.dataset.gid].forEach((el) =>
        el.toggleAttribute("on", false)
      );
      current = null;
      if (broadcast) {
