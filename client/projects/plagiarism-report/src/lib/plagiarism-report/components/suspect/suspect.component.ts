...
@Component({
  ...
  styleUrls: ["./suspect.component.sass"],
  animations: [fadeIn],
})
export class SuspectComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    suspect$.pipe(untilDestroy(this), truthy()).subscribe((item) => {
      ...
      suspectPreview$
        .pipe(untilDestroy(this), truthy(), take(1))
        .subscribe((preview) => {
    ...
    onlyOneToOne$
      .pipe(untilDestroy(this))
      .subscribe((disable) => (this.disableBackButton = disable));
    this.layoutService.isMobile$
      .pipe(untilDestroy(this))
      .subscribe((value) => (this.isMobile = value));
    this.matchService.suspectTextMatches$
      .pipe(untilDestroy(this))
