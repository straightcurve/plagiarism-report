...
export class PropertiesComponent implements OnInit, OnDestroy {
  ...
  get combined() {
    return (
      this.stats.identical + this.stats.relatedMeaning + this.stats.minorChanges
    );
  }
  get score() {
    const res = Math.min(
      1,
      this.combined / (this.stats.total - this.stats.omittedWords)
    );
  ...
  get isShowingPartialStats() {
    ...
    if (
      !this.options.showIdentical &&
      this.metadata.results.score.identicalWords > 0
    ) {
      return true;
    }
    if (
      !this.options.showMinorChanges &&
      this.metadata.results.score.minorChangedWords > 0
    ) {
      return true;
    }
    if (
      !this.options.showRelated &&
      this.metadata.results.score.relatedMeaningWords > 0
    ) {
      return true;
    }
    return false;
    ...
    if (this.scanSummaryComponent) {
      if (!this.scanSummeryComponentInstance) {
        setTimeout(() => {
          ...
          if (!this.scanSummeryComponentInstance) {
            const scanSummeryComponentInstance =
              this.scanSammeryComponentVcr?.createComponent(factory);
            if (scanSummeryComponentInstance) {
              this.scanSummeryComponentInstance = scanSummeryComponentInstance;
              if (this.metadata) {
                this.scanSummeryComponentInstance?.instance?.setCompleteResult(
                  this.metadata
                );
              }
            }
  ...
  ngOnInit() {
    ...
    previews$
      ...
      .subscribe((previews) => (this.previewCount = previews.length));
    hiddenResults$
      .pipe(untilDestroy(this))
      .subscribe(
        (hiddneResults) => (this.hiddenResultsCount = hiddneResults.length)
      );
    ...
    this.statistics.statistics$
      ...
      .subscribe((value) => {
        this.stats = value;
        const { identical, minorChanges, relatedMeaning, omittedWords, total } =
          value;
        this.chartData = [
          ...
          { name: "Related meaning", value: relatedMeaning },
          {
            ...
            value:
              total -
              (identical + minorChanges + relatedMeaning + omittedWords),
          },
        ];
      });
   ...
   */
  openSettingsDialog() {
    this.dialogService.open<OptionsDialogComponent, ReportService>(
