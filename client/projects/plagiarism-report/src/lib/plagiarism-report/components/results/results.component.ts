...
export class ResultsComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    console.log("results component report", reportService.guid);
  }
  ...
  public get visibleResults() {
    return this.focusedMatch.ids.filter(
      (id) => !this.hiddenResults.includes(id)
    );
  }
   ...
   */
  openFilterDialog() {
    this.dialogService.open<ResultsFilterDialogComponent, ReportService>(
  ...
  ngOnInit() {
    ...
    const { filteredPreviews$, hiddenResults$, contentMode$, results$ } =
      this.reportService;
    ...
    this.copyleaksService.onReportConfig$
      .pipe(untilDestroy(this))
      .subscribe((config) => {
  ...
  private checkAndAddOverlay(config: CopyleaksReportConfig) {
    if (config && config.resultsOverlayComponent) {
      ...
      if (!this.resultsOverlayComponentInstance) {
        ...
        setTimeout(() => {
          if (!this.resultsOverlayComponentInstance) {
            const resultsOverlayComponentInstance =
              this.vcr?.createComponent(factory);
            if (resultsOverlayComponentInstance) {
              this.resultsOverlayComponentInstance =
                resultsOverlayComponentInstance;
            }
          }
        }, 100);
      }
