...
export class SourceTextHelperDirective implements AfterContentInit, OnDestroy {
  ...
  ngAfterContentInit() {
    ...
    const { textMatchClick$, jump$, sourceText$, suspectHtml$ } =
      this.highlightService;
    sourceText$
      .pipe(untilDestroy(this))
      .subscribe((value) => (this.current = value));
    textMatchClick$
      .pipe(
        untilDestroy(this),
    ...
    jump$
      .pipe(
        ...
        filter(
          ([, view, content]) => view === "one-to-one" && content === "text"
        )
      )
    ...
    suspectHtml$
      ...
      .subscribe(([match, source, suspect]) =>
        this.handleBroadcast(match, source, suspect.result, "html")
      );
  }
  /**
  ...
  handleBroadcast(
    ...
    const [, start] = helpers.findRespectiveMatch(
      match,
      suspect[contentMode].comparison,
      false
    );
    const page = helpers.findRespectivePage(
      start,
  ...
  handleJump(forward: boolean) {
    if (this.canJumpInCurrentPage(forward)) {
      ...
      const nextIndex = this.current
        ? components.indexOf(this.current) + (forward ? 1 : -1)
        : 0;
      this.highlightService.textMatchClicked({
        elem: components[nextIndex],
        broadcast: true,
        origin: "source",
      });
    } else {
      const page = forward
        ? helpers.findNextPageWithMatch(
            this.host.textMatches,
            this.host.currentPage
          )
        : helpers.findPrevPageWithMatch(
            this.host.textMatches,
            this.host.currentPage
          );
