...
export class OriginalTextHelperDirective implements AfterContentInit, OnDestroy {
  ...
  handleJump(forward: boolean) {
    if (this.canJumpInCurrentPage(forward)) {
      ...
      const page = (
        forward ? helpers.findNextPageWithMatch : helpers.findPrevPageWithMatch
      )(this.host.textMatches, this.host.currentPage);
  ...
  ngAfterContentInit() {
    ...
    jump$
      .pipe(
        ...
        filter(
          ([, view, content]) => view === "one-to-many" && content === "text"
        )
      )
    ...
    textMatchClick$
      .pipe(
        ...
        withLatestFrom(viewMode$, contentMode$),
        filter(
          ([textMatchClickEvent, view, content]) =>
            textMatchClickEvent && view === "one-to-many" && content === "text"
        )
      )
