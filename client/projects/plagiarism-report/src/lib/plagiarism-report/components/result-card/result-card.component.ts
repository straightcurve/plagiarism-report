...
@Component({
  ...
  styleUrls: ["./result-card.component.sass"],
  providers: [DatePipe],
})
export class ResultCardComponent implements OnInit, OnDestroy {
  ...
  public popoverHoverDelay: number = 0;
  ...
  ngOnInit() {
    ...
    if (
      this.preview?.metadata?.author ||
      this.preview?.metadata?.organization ||
      this.previewDate
    ) {
      ...
      this.metaDataToolTip = `${author ? author : ""}${
        author && organization ? " • " : ""
      }${organization ? organization : ""}${
        (author || organization) && previewDate ? " • " : ""
      }${previewDate ? previewDate : ""}`;
    }

    if (this.preview.component) {
      const factory =
        this.componentFactoryResolver.resolveComponentFactory<ResultPreviewComponentBase>(
          this.preview.component
        );
      setTimeout(() => {
        const ref =
          this.vcr?.createComponent<ResultPreviewComponentBase>(factory);
        if (ref) {
    ...
    this.similarWords$ = combineLatest([result$, options$]).pipe(
      ...
      map(([result, options]) => {
        if (result && result.result && options) {
          ...
          const { identical, minorChanges, relatedMeaning } =
            result.result.statistics;
          return (
            (showIdentical ? identical : 0) +
            (showMinorChanges ? minorChanges : 0) +
            (showRelated ? relatedMeaning : 0)
          );
        }
