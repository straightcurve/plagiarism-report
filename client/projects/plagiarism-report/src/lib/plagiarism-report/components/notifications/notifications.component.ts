...
export class NotificationsComponent implements OnInit, OnDestroy {
  ...
  @Output() severityChange =
    new EventEmitter<CompleteResultNotificationAlertSeverity>();
  constructor(
      ...
      .pipe(
        ...
        filter(
          (c) =>
            ...
            !!c.notifications.alerts.length
        )
      )
      .subscribe((completeResult) => {
        const alertSeverities = completeResult.notifications.alerts.map(
          (s) => +s.severity
        );
