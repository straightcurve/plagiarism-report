...
export class ReportService implements OnDestroy {
  ...
  // * configurable state
  private _config = new BehaviorSubject<CopyleaksReportConfig>({
  ...
  constructor(
    ...
    console.log("report", this.guid);
    console.log("constructor", copyleaksService.guid);
    onCompleteResult$
      .pipe(untilDestroy(this))
      .subscribe((completeResult) => this.setCompleteResult(completeResult));
    onResultPreview$
      .pipe(untilDestroy(this))
      .subscribe((preview) => this.addPreview(preview));
    onProgress$
      .pipe(untilDestroy(this))
      .subscribe((progress) => this.setProgress(progress));
    onTotalResultsChange$
      .pipe(untilDestroy(this))
      .subscribe((totalResults) => this.setTotalResults(totalResults));
    onResultItems$
      .pipe(untilDestroy(this))
      .subscribe((resultItem) => this.addDownloadedResults(resultItem));
    onScanSource$
      .pipe(untilDestroy(this))
      .subscribe((source) => this.setSource(source));
    onReportConfig$
      .pipe(untilDestroy(this))
      .subscribe((config) => this.configure(config));
    this.config$
      .pipe(untilDestroy(this))
      .subscribe((config) => this._configChange.next(config));
    ...
    onDeleteResultById$.pipe(untilDestroy(this)).subscribe((id) => {
      ...
      this.setCompleteResult({
        ...
        results: {
          ...
          internet: currentCompleteResult.results.internet.filter(
            (r) => r.id !== id
          ),
          database: currentCompleteResult.results.database.filter(
            (r) => r.id !== id
          ),
          repositories: currentCompleteResult.results.repositories?.filter(
            (r) => r.id !== id
          ),
          score: {
            ...currentCompleteResult.results.score,
            aggregatedScore: statistics.aggregatedScore || 0,
  ...
  public completeResult$: Observable<CompleteResult> = this._completeResult
    .asObservable()
    .pipe(truthy());
  ...
  public resultCardActions$ = this.config$.pipe(
    map((x) => x.resultCardActions)
  );
  public onlyOneToOne$ = this.config$.pipe(
    map((x) => x.disableSuspectBackButton)
  );
  ...
  public hiddenResults$ = this._hiddenResults
    .asObservable()
    .pipe(distinctUntilChanged());
  public results$ = this._results.asObservable().pipe(truthy());
  public previews$ = this._previews.asObservable().pipe(truthy());

  public filteredPreviews$ = combineLatest([
    this.previews$,
    this.hiddenResults$,
  ]).pipe(
    map(([results, ids]) =>
      results.filter((result) => !ids.includes(result.id))
    )
  );
  public filteredResults$ = combineLatest([
    this.results$,
    this.hiddenResults$,
  ]).pipe(
    map(([results, ids]) =>
  ...
  public setCompleteResult(completeResult: CompleteResult) {
    ...
    if (completeResult.filters && completeResult.filters.resultIds) {
      this.copyleaksService.setFilteredResultsIds(
        completeResult.filters.resultIds
      );
    }
  ...
  public setSource(source: ScanSource) {
    ...
    if (
      source &&
      (!source.html || !source.html.value) &&
      this._config.value.contentMode === "html"
    ) {
  ...
  public upgradePlanEvent() {
    ...
    if (completeResult?.scannedDocument?.expectedCredits) {
      recommendedPagesAmount =
        completeResult?.scannedDocument?.expectedCredits -
        completeResult.scannedDocument?.credits;
    }
  ...
  public addPreview(preview: ResultPreview) {
    if (
      !this._completeResult.value &&
      !(this._previews.value || []).find((p) => p.id === preview.id)
    ) {
  ...
  public addDownloadedResults(resultItems: ResultItem[]) {
    const newItems = resultItems.filter(
      (res) => !this._results.value.find((x) => x.id === res.id)
    );
    if (newItems.length) {
