...
export class StatisticsService implements OnDestroy {
  ...
  constructor(reportService: ReportService) {
    ...
    combineLatest([completeResult$, suspect$, options$, viewMode$])
      .pipe(
        untilDestroy(this),
        filter(
          ([, suspect, , viewMode]) => viewMode === "one-to-one" && !!suspect
        )
      )
  ...
  retreiveOneToOneStatistics(
    ...
    this._statistics.next({
      identical: options.showIdentical
        ? suspect.result.statistics.identical
        : 0,
      relatedMeaning: options.showRelated
        ? suspect.result.statistics.relatedMeaning
        : 0,
      minorChanges: options.showMinorChanges
        ? suspect.result.statistics.minorChanges
        : 0,
  ...
  retreieveOneToManyStatistics(
    ...
    const totalResults =
      (completeResult.results.repositories &&
    ...
    const showAll =
      options.showIdentical && options.showMinorChanges && options.showRelated;
    const missingAggregated =
      totalResults !== 0 && completeResult.results.score.aggregatedScore === 0;
    let stats: ReportStatistics;
    if (
      (!completeResult.filters ||
        !completeResult.filters.resultIds ||
        !completeResult.filters.resultIds.length) &&
