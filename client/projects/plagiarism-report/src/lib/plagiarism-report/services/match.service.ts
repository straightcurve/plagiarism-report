...
export class MatchService implements OnDestroy {
  ...
  constructor(
    ...
    const { source$, filteredResults$, options$, previews$, hiddenResults$ } =
      this.reportService;
    ...
    combineLatest([
      ...
      .subscribe(([result, options, source]) =>
        this.processOneToOneMatches(result, options, source)
      );
    ...
    combineLatest([
      ...
      .pipe(untilDestroy(this))
      .subscribe(
        ([results, previews, hiddenResults, options, source, viewMode]) => {
          if (_timeout) {
          ...
          if (viewMode === EReportViewModel.Alerts) {
            ...
            _timeout = setTimeout(
              ...
              results.length === previews.length - hiddenResults.length
                ? 0
                : 5000
            );
          }
  ...
  private get onSourceFirstHtmlMode$() {
    ...
    return combineLatest([contentMode$, source$]).pipe(
      ...
      filter(
        ([content, source]) =>
          content === "html" && source.html && !!source.html.value
      ),
      take(1)
    );
  }
  ...
  private get onSuspectChange$() {
    return this.reportService.suspectResult$.pipe(
      untilDestroy(this),
      truthy(),
      distinctUntilChanged()
    );
  ...
  public get sourceHtmlMatches$() {
    return this._sourceHtmlMatches
      .asObservable()
  ...
  public get suspectTextMatches$() {
    return this._suspectTextMatches
      .asObservable()
      .pipe(untilDestroy(this), truthy());
  }
  ...
  public get suspectHtmlMatches$() {
    return this._suspectHtmlMatches
      .asObservable()
      .pipe(untilDestroy(this), truthy());
  }
  ...
  public get originalTextMatches$() {
    return this._originalTextMatches
      .asObservable()
      .pipe(untilDestroy(this), truthy());
  }
  ...
  public get originalHtmlMatches$() {
    return this._originalHtmlMatches
      .asObservable()
      .pipe(untilDestroy(this), truthy());
  }
  ...
  private processOneToOneMatches(
    ...
    if (
      ...
      this.onSuspectContentModeChange$
        .pipe(takeUntil(this.onNewSuspect$))
        .subscribe((mode) => {
          const text = helpers.processSuspectText(
            item,
            settings,
            mode === "text"
          );
  ...
  private processAlertMatches(
    ...
    this.onSourceFirstTextMode$.subscribe(() => {
      ...
      if (
        this.viewModeService?.selectedAlert?.code ===
        ALERTS.SUSPECTED_CHARACTER_REPLACEMENT_CODE
      ) {
        text = helpers.processSuspectedCharacterMatches(
          source,
          this.viewModeService.selectedAlert
        );
      } else {
