...
export const mergeWords = (matches: Match[]): Match[] => {
  return findNests(matches).reduce(
    (all, nest) => all.concat(mergeWordIntervals(nest)),
    []
  );
};
...
export const calculateStatistics = (
  ...
  const identical = options.showIdentical
    ? results.flatMap(createWordIntervalsFrom("identical", "source"))
    : [];
  ...
  const relatedMeaning = options.showRelated
    ...
    : [];
  const withOutoverlaps = mergeWords([
    ...relatedMeaning,
    ...minorChanges,
    ...identical,
  ]);
  ...
  let aggregatedScore = Math.min(
    1,
    (identicalCount + relatedMeaningCount + minorChangesCount) /
      (totalWords - totalExcluded)
  );
