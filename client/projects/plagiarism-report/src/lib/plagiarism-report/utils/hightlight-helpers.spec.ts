...
describe("Highlight Helpers", () => {
  ...
  describe("findPartial", () => {
    ...
    it(`should find the correct start position`, () => {
      expect(
        helpers.findPartialMatch({ start: 0, end: 25 } as Match, comparisons)
      ).toBe(0);
      expect(
        helpers.findPartialMatch({ start: 25, end: 49 } as Match, comparisons)
      ).toBe(0);
      expect(
        helpers.findPartialMatch({ start: 50, end: 75 } as Match, comparisons)
      ).toBe(50);
      expect(
        helpers.findPartialMatch({ start: 75, end: 99 } as Match, comparisons)
      ).toBe(50);
      expect(
        helpers.findPartialMatch({ start: 100, end: 500 } as Match, comparisons)
      ).toBe(100);
      expect(
        helpers.findPartialMatch(
          { start: 500, end: 1100 } as Match,
          comparisons
        )
      ).toBe(100);
    });
  ...
  describe("findRespectiveMatch", () => {
    ...
    it(`should find the correct start position`, () => {
      expect(
        helpers.findRespectiveMatch(
          { type, start: 0 } as Match,
          comparisons,
          true
        )
      ).toEqual([0, 70]);
