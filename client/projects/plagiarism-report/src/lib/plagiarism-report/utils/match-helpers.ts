...
const extractMatches =
  (comparison: ComparisonKey, content: ContentKey, subject: SubjectResultKey) =>
  (item: ResultItem) => {
    ...
    const { starts, lengths } =
      result[content].comparison[comparison][subject].chars;
    return starts.map(
      (start, i): Match => ({
        start,
        end: start + lengths[i],
...
export const sourceTextIdentical = extractMatches(
  "identical",
  "text",
  "source"
);
...
export const sourceTextRelatedMeaning = extractMatches(
  "relatedMeaning",
  "text",
  "source"
);
...
export const sourceTextMinorChanges = extractMatches(
  "minorChanges",
  "text",
  "source"
);
...
export const sourceHtmlIdentical = extractMatches(
  "identical",
  "html",
  "source"
);
...
export const sourceHtmlRelatedMeaning = extractMatches(
  "relatedMeaning",
  "html",
  "source"
);
...
export const sourceHtmlMinorChanges = extractMatches(
  "minorChanges",
  "html",
  "source"
);
...
export const suspectTextIdentical = extractMatches(
  "identical",
  "text",
  "suspected"
);
...
export const suspectTextRelatedMeaning = extractMatches(
  "relatedMeaning",
  "text",
  "suspected"
);
...
export const suspectTextMinorChanges = extractMatches(
  "minorChanges",
  "text",
  "suspected"
);
...
export const suspectHtmlIdentical = extractMatches(
  "identical",
  "html",
  "suspected"
);
...
export const suspectHtmlRelatedMeaning = extractMatches(
  "relatedMeaning",
  "html",
  "suspected"
);
...
export const suspectHtmlMinorChanges = extractMatches(
  "minorChanges",
  "html",
  "suspected"
);
 ...
 */
export const findRespectiveStart = (
  index: number,
  ...
  const found = from.chars.starts.findIndex(
    (s, i) => s <= index && index <= s + source.chars.lengths[i]
  );
...
export const processSourceText = (
  ...
  const identical = settings.showIdentical
    ? items.reduce(
        (acc, res) =>
          acc.concat((text ? sourceTextIdentical : sourceHtmlIdentical)(res)),
        []
      )
    : [];
  const minor = settings.showMinorChanges
    ? items.reduce(
        (acc, res) =>
          acc.concat(
            (text ? sourceTextMinorChanges : sourceHtmlMinorChanges)(res)
          ),
        []
      )
    : [];
  const related = settings.showRelated
    ? items.reduce(
        (acc, res) =>
          acc.concat(
            (text ? sourceTextRelatedMeaning : sourceHtmlRelatedMeaning)(res)
          ),
        []
      )
    : [];
  const excluded = sourceTextExcluded(source);
  const grouped = mergeMatches([
    ...identical,
    ...minor,
    ...related,
    ...excluded,
  ...
  return paginateMatches(
    source.text.value,
    source.text.pages.startPosition,
    filled
  );
};
...
export const processSuspectText = (
  ...
  const identical = options.showIdentical
    ? (text ? suspectTextIdentical : suspectHtmlIdentical)(suspect)
    : [];
  const minor = options.showMinorChanges
    ? (text ? suspectTextMinorChanges : suspectHtmlMinorChanges)(suspect)
    : [];
  const related = options.showRelated
    ? (text ? suspectTextRelatedMeaning : suspectHtmlRelatedMeaning)(suspect)
    : [];
...
export const processSourceHtml = (
  ...
  const items = [].concat(results);
  const identical = options.showIdentical
    ? items.reduce((acc, res) => acc.concat(sourceHtmlIdentical(res)), [])
    : [];
  const minor = options.showMinorChanges
    ? items.reduce((acc, res) => acc.concat(sourceHtmlMinorChanges(res)), [])
    : [];
  const related = options.showRelated
    ? items.reduce((acc, res) => acc.concat(sourceHtmlRelatedMeaning(res)), [])
    : [];
 ...
 * @param options the current user options
 */
export const processSuspectHtml = (
  suspect: ResultItem,
  options: CopyleaksReportOptions
): Match[] => {
  ...
  const minor = options.showMinorChanges
    ? suspectHtmlMinorChanges(suspect)
    : [];
