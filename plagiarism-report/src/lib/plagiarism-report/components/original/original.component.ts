...
} from "../../services/copyleaks-translate.service";
import {
  EReportViewModel,
  ViewModeService,
...
@Component({
  ...
  styleUrls: ["./original.component.sass"],
  animations: [fadeIn],
})
export class OriginalComponent implements OnInit, OnDestroy {
  ...
  get AlertPreviewViewMode() {
    return (
      this.viewModeService?.reportViewMode$?.value ===
        EReportViewModel.Alerts &&
  ...
  toggleContent() {
    this.reportService.configure({
      contentMode: this.isHtml ? "text" : "html",
    });
  }
  ...
  ngOnInit() {
    ...
    if (
      this.translations &&
      this.translations.SCAN_SETTINGS &&
      this.translations.SCAN_SETTINGS.OMITTED
    ) {
    ...
    const { completeResult$, source$, viewMode$, contentMode$, sourcePage$ } =
      this.reportService;
    const {
      originalTextMatches$,
      sourceTextMatches$,
    ...
    completeResult$
      ...
      .subscribe((completeResult) => (this.completeResult = completeResult));
    ...
    viewMode$
      .pipe(untilDestroy(this), distinctUntilChanged(), switchMapTo(source$))
      .subscribe((source) => {
    ...
    contentMode$
      .pipe(untilDestroy(this))
      .subscribe((content) => (this.contentMode = content));
    originalTextMatches$
      .pipe(untilDestroy(this))
      .subscribe((matches) => (this.originalTextMatches = matches));
    sourceTextMatches$
      .pipe(untilDestroy(this))
      .subscribe((matches) => (this.sourceTextMatches = matches));
    originalHtmlMatches$
      .pipe(untilDestroy(this))
      .subscribe((matches) => (this.originalHtmlMatches = matches));
    sourceHtmlMatches$
      .pipe(untilDestroy(this))
      .subscribe((matches) => (this.sourceHtmlMatches = matches));
    this.layoutService.mediaAliases$
      .pipe(untilDestroy(this))
      .subscribe((queries) => (this.activeMediaQueries = queries));
  }
  ...
  openUrl() {
    if (this.hasUrl) {
      window.open(
        this.completeResult?.scannedDocument?.metadata?.finalUrl as string,
        "_blank"
      );
    }
  }
