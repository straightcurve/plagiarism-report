...
@Component({
  ...
  template: "",
  styleUrls: ["./html-helper.sass"],
})
export class SuspectHtmlHelperComponent
  extends HtmlHelperBase
  implements OnInit, OnDestroy
{
  ...
  handleMatchSelect(event: MatchSelectEvent) {
    this.highlightService.setSuspectHtmlMatch(
      event.index !== -1 ? this.matches[event.index] : null
    );
  }
  ...
  ngOnInit() {
    ...
    suspect$
      ...
      .subscribe(
        (suspect) =>
          suspect.result &&
          suspect.result.html &&
          (this.html = suspect.result.html.value)
      );
    suspectHtmlMatches$
      .pipe(untilDestroy(this))
      .subscribe((matches) => this.renderMatches(matches));
    // jump$
    // 	.pipe(
    // 		untilDestroy(this),
    ...
    textMatchClick$
      ...
      .subscribe(([elem, suspect, matches]) => {
        if (elem && suspect && suspect.result) {
          const comparison =
            suspect.result.html.comparison[MatchType[elem.match.type]];
          const [start] = findRespectiveStart(
            elem.match.start,
            comparison,
            true
          );
    ...
    sourceHtml$
      .pipe(
        ...
        filter(
          ([, suspect]) =>
            suspect && suspect.result && !!suspect.result.html.value
        )
      )
      .subscribe(([match, suspect, matches]) => {
        if (match && suspect && suspect.result) {
          const comparison =
            suspect.result.html.comparison[MatchType[match.type]];
